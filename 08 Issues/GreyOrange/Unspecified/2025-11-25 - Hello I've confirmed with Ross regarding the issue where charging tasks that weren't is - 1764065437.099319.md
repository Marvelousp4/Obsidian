---
type: "issue"
source_type: "support_history_import"
client: "GreyOrange"
account: "GreyOrange"
project: ""
site: "Unspecified"
vendor: "Wellwit Robotics"
category: "support_coordination"
item_type: "coordination"
status: "open"
severity: ""
priority: "P4"
created_date: "2025-11-25"
last_updated_date: "2025-11-25"
reporter: "U06GFL86XUP"
assignee: ""
source_id: "1764065437.099319"
bot_ids: []
components: []
tags: []
---

# Hello I've confirmed with Ross regarding the issue where charging tasks that weren't is...

## Source Context

- Imported from: /Users/bai/Documents/Playground/wellwit_issue_history_clean.csv
- Source channel/file: C08GU4VBN1G_2025-01-01_to_2026-03-16.txt
- Original source id: 1764065437.099319
- Site: Unspecified
- Vendor / platform context: Wellwit Robotics

## Problem

Hello I've confirmed with Ross regarding the issue where charging tasks that weren't issued still appeared automatically. We need you to provide the bot logs at the time the problem occurred for our analysis.

## Progress

No progress note captured in the cleaned source.

## Next Action

No explicit next action found in the cleaned source.

## Metadata

- Category: support_coordination
- Item type: coordination
- Status: open
- Severity: 
- Priority: P4
- Created: 2025-11-25
- Last updated: 2025-11-25
- Bot IDs: 
- Components: 

## Related Links

- Account: [[04 Clients/GreyOrange|GreyOrange]]
- Site note: 
- Vendor note: [[04 Clients/Wellwit Robotics|Wellwit Robotics]]
