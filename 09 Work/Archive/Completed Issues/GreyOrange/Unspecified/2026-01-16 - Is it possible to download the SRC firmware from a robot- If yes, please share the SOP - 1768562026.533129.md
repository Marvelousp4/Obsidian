---
type: "issue"
source_type: "support_history_import"
account: "GreyOrange"
project: ""
site: "Unspecified"
vendor: "Wellwit Robotics"
category: "software_or_firmware"
item_type: "software"
status: "resolved"
severity: ""
priority: "P4"
created_date: "2026-01-16"
last_updated_date: "2026-04-07"
reporter: "U046B9XUZRV"
assignee: ""
source_id: "1768562026.533129"
source_file: "wellwit_issue_history_clean.csv"
bot_ids: []
components: ["src"]
tags: []
---

# Is it possible to download the SRC firmware from a robot? If yes, please share the SOP....

## Source Context

- Imported from: wellwit_issue_history_clean.csv
- Source channel/file: C06J8L4SG64_2026.txt
- Original source id: 1768562026.533129
- Site: Unspecified
- Vendor / platform context: Wellwit Robotics

## Problem

Is it possible to download the SRC firmware from a robot? If yes, please share the SOP. FYI

## Progress

Please respond. FYI We can't

## Next Action

No explicit next action found in the cleaned source.

## Metadata

- Category: software_or_firmware
- Item type: software
- Status: resolved
- Severity: 
- Priority: P4
- Created: 2026-01-16
- Last updated: 2026-01-19
- Bot IDs: 
- Components: src

## Related Links

- Account: [[09 Work/Accounts/GreyOrange|GreyOrange]]
- Site note: 
- Vendor note: [[09 Work/Accounts/Wellwit Robotics|Wellwit Robotics]]
