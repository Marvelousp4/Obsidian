---
type: "issue"
source_type: "support_history_import"
account: "GreyOrange"
project: ""
site: "Unspecified"
vendor: "Wellwit Robotics"
category: "hardware_or_mechanical"
item_type: "hardware"
status: "resolved"
severity: ""
priority: "P4"
created_date: "2025-05-13"
last_updated_date: "2026-04-07"
reporter: "U04580WJL65"
assignee: ""
source_id: "1747180772.735929"
source_file: "wellwit_issue_history_clean.csv"
bot_ids: []
components: ["sto"]
tags: []
---

# Robots are stopping unnecessarily, even we don't have any other bot nearby check robot...

## Source Context

- Imported from: wellwit_issue_history_clean.csv
- Source channel/file: C08GU4VBN1G_2025-01-01_to_2026-03-16.txt
- Original source id: 1747180772.735929
- Site: Unspecified
- Vendor / platform context: Wellwit Robotics

## Problem

Robots are stopping unnecessarily, even we don't have any other bot nearby check robot near to AP10067

## Progress

This is caused by robot being disconnected.

## Next Action

It needs robot logs as well, shared robot log with

## Metadata

- Category: hardware_or_mechanical
- Item type: hardware
- Status: resolved
- Severity: 
- Priority: P4
- Created: 2025-05-13
- Last updated: 2025-05-13
- Bot IDs: 
- Components: sto

## Related Links

- Account: [[09 Work/Accounts/GreyOrange|GreyOrange]]
- Site note: 
- Vendor note: [[09 Work/Accounts/Wellwit Robotics|Wellwit Robotics]]
