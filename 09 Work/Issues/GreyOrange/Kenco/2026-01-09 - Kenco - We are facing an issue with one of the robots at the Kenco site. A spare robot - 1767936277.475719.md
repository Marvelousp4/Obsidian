---
type: "issue"
source_type: "support_history_import"
account: "GreyOrange"
project: "Kenco"
site: "Kenco"
vendor: "Wellwit Robotics"
category: "support_coordination"
item_type: "coordination"
status: "open"
severity: ""
priority: "P4"
created_date: "2026-01-09"
last_updated_date: "2026-01-09"
reporter: "U046B9XUZRV"
assignee: ""
source_id: "1767936277.475719"
source_file: "wellwit_issue_history_clean.csv"
bot_ids: []
components: []
tags: []
---

# Kenco | We are facing an issue with one of the robots at the Kenco site. A spare robot...

## Source Context

- Imported from: wellwit_issue_history_clean.csv
- Source channel/file: C0602U5PLMS_2026.txt
- Original source id: 1767936277.475719
- Site: Kenco
- Vendor / platform context: Wellwit Robotics

## Problem

We are facing an issue with one of the robots at the Kenco site. A spare robot was sent recently, but it’s not powering on. We’ve done basic troubleshooting and checked all wire connections, but couldn’t conclude the root cause. Could someone from your team please be available to help debug this? We’re planning a call today at 9:00 PM IST and will share the meeting details shortly.

## Progress

[THREAD] :arrow_down:

## Next Action

No explicit next action found in the cleaned source.

## Metadata

- Category: support_coordination
- Item type: coordination
- Status: open
- Severity: 
- Priority: P4
- Created: 2026-01-09
- Last updated: 2026-01-09
- Bot IDs: 
- Components: 

## Related Links

- Account: [[09 Work/Accounts/GreyOrange|GreyOrange]]
- Site note: [[04 Projects/Work/GreyOrange/Kenco.md|Kenco]]
- Vendor note: [[09 Work/Accounts/Wellwit Robotics|Wellwit Robotics]]
