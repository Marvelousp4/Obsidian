---
type: "issue"
source_type: "support_history_import"
account: "GreyOrange"
project: "GFC"
site: "GFC"
vendor: "Wellwit Robotics"
category: "software_or_firmware"
item_type: "software"
status: "resolved"
severity: "SEV2"
priority: "P2"
created_date: "2026-01-13"
last_updated_date: "2026-04-07"
reporter: "U074K0MQT4N"
assignee: ""
source_id: "1768323784.227939"
source_file: "wellwit_issue_history_clean.csv"
bot_ids: []
components: ["license", "rds"]
tags: []
---

# GFC-2 | - At GFC-2 We are having an RDS where it was inactivated.

## Source Context

- Imported from: wellwit_issue_history_clean.csv
- Source channel/file: C0602U5PLMS_2026.txt
- Original source id: 1768323784.227939
- Site: GFC
- Vendor / platform context: Wellwit Robotics

## Problem

- At GFC-2 We are having an RDS where it was inactivated. Can you please help us to activate the RDS.

## Progress

-FYI - Can you help us to resolve this issue. As it was an blocker for us - we are facing this issue when we upload license

## Next Action

Check here for the license

## Metadata

- Category: software_or_firmware
- Item type: software
- Status: resolved
- Severity: SEV2
- Priority: P2
- Created: 2026-01-13
- Last updated: 2026-01-16
- Bot IDs: 
- Components: license, rds

## Related Links

- Account: [[09 Work/Accounts/GreyOrange|GreyOrange]]
- Site note: [[04 Projects/Work/GreyOrange/GFC.md|GFC]]
- Vendor note: [[09 Work/Accounts/Wellwit Robotics|Wellwit Robotics]]
