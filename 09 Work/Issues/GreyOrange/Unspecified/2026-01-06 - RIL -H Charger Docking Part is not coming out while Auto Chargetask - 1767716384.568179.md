---
type: "issue"
source_type: "support_history_import"
account: "GreyOrange"
project: ""
site: "Unspecified"
vendor: "Wellwit Robotics"
category: "hardware_or_mechanical"
item_type: "hardware"
status: "open"
severity: ""
priority: "P4"
created_date: "2026-01-06"
last_updated_date: "2026-01-06"
reporter: "U0492VBFW48"
assignee: ""
source_id: "1767716384.568179"
source_file: "wellwit_issue_history_clean.csv"
bot_ids: ["9016"]
components: ["charger", "dock"]
tags: []
---

# RIL -H Charger Docking Part is not coming out while Auto Chargetask

## Source Context

- Imported from: wellwit_issue_history_clean.csv
- Source channel/file: C0602U5PLMS_2026.txt
- Original source id: 1767716384.568179
- Site: Unspecified
- Vendor / platform context: Wellwit Robotics

## Problem

RIL -H Charger Docking Part is not coming out while Auto Chargetask

## Progress

As per conversation we are sharing the logs for RIL-H 9016 where it was having the Auto Chargetask and the charger docking port was not coming out

## Next Action

No explicit next action found in the cleaned source.

## Metadata

- Category: hardware_or_mechanical
- Item type: hardware
- Status: open
- Severity: 
- Priority: P4
- Created: 2026-01-06
- Last updated: 2026-01-06
- Bot IDs: 9016
- Components: charger, dock

## Related Links

- Account: [[09 Work/Accounts/GreyOrange|GreyOrange]]
- Site note: 
- Vendor note: [[09 Work/Accounts/Wellwit Robotics|Wellwit Robotics]]
