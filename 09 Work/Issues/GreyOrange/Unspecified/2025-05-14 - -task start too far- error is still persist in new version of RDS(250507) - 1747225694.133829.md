---
type: "issue"
source_type: "support_history_import"
account: "GreyOrange"
project: ""
site: "Unspecified"
vendor: "Wellwit Robotics"
category: "hardware_or_mechanical"
item_type: "hardware"
status: "resolved"
severity: ""
priority: "P4"
created_date: "2025-05-14"
last_updated_date: "2026-04-07"
reporter: "U04580WJL65"
assignee: ""
source_id: "1747225694.133829"
source_file: "wellwit_issue_history_clean.csv"
bot_ids: []
components: ["rds", "sto"]
tags: []
---

# "task start too far" error is still persist in new version of RDS(250507)

## Source Context

- Imported from: wellwit_issue_history_clean.csv
- Source channel/file: C08GU4VBN1G_2025-01-01_to_2026-03-16.txt
- Original source id: 1747225694.133829
- Site: Unspecified
- Vendor / platform context: Wellwit Robotics

## Problem

"task start too far" error is still persist in new version of RDS(250507)

## Progress

Getting this issue with this image 📎image (1).png U0ALSB28Q0Y/F08T0FVMXG9 This is a warning. Path planning procedure is not stopped. ok, we will test

## Next Action

Ok. I'll check this. This version disabled the check causing 52201

## Metadata

- Category: hardware_or_mechanical
- Item type: hardware
- Status: resolved
- Severity: 
- Priority: P4
- Created: 2025-05-14
- Last updated: 2025-05-18
- Bot IDs: 
- Components: rds, sto

## Related Links

- Account: [[09 Work/Accounts/GreyOrange|GreyOrange]]
- Site note: 
- Vendor note: [[09 Work/Accounts/Wellwit Robotics|Wellwit Robotics]]
