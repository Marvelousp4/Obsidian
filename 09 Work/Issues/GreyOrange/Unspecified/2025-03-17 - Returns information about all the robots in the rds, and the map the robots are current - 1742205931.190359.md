---
type: "issue"
source_type: "support_history_import"
client: "GreyOrange"
account: "GreyOrange"
project: ""
site: "Unspecified"
vendor: "Wellwit Robotics"
category: "software_or_firmware"
item_type: "software"
status: "open"
severity: ""
priority: "P4"
created_date: "2025-03-17"
last_updated_date: "2025-03-17"
reporter: "U06GFL7R1TR"
assignee: ""
source_id: "1742205931.190359"
bot_ids: []
components: ["rds"]
tags: []
---

# Returns information about all the robots in the rds, and the map the robots are current...

## Source Context

- Imported from: /Users/bai/Documents/Playground/wellwit_issue_history_clean.csv
- Source channel/file: C06TL405CRF_2025-01-01_to_2026-03-16.txt
- Original source id: 1742205931.190359
- Site: Unspecified
- Vendor / platform context: Wellwit Robotics

## Problem

Returns information about all the robots in the rds, and the map the robots are currently using

## Progress

No progress note captured in the cleaned source.

## Next Action

No explicit next action found in the cleaned source.

## Metadata

- Category: software_or_firmware
- Item type: software
- Status: open
- Severity: 
- Priority: P4
- Created: 2025-03-17
- Last updated: 2025-03-17
- Bot IDs: 
- Components: rds

## Related Links

- Account: [[09 Work/Accounts/GreyOrange|GreyOrange]]
- Site note: 
- Vendor note: [[09 Work/Accounts/Wellwit Robotics|Wellwit Robotics]]
